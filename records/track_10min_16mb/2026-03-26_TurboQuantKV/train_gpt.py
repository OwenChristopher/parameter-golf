...
from turboquant_kv import eval_val_turboquant
class Hyperparameters:
    ...
    tq_enabled = bool(int(os.environ.get("TQ_EVAL", "1")))
...
def main() -> None:
    ...
    if args.tq_enabled:
        ...
        log0(f"turboquant_kv val_loss:{tq_loss:.4f} val_bpb:{tq_bpb:.4f} "
             f"bits:{args.tq_bits} eval_time:{1000.0*(time.perf_counter()-t_tq):.0f}ms")
        log0(f"turboquant_kv_exact val_loss:{tq_loss:.8f} val_bpb:{tq_bpb:.8f}")
    if distributed:
